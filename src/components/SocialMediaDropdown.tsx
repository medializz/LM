...
import { Share2, ChevronUp, ChevronDown, ExternalLink, Globe } from 'lucide-react';
...
export const SocialMediaDropdown: React.FC<SocialMediaDropdownProps> = ({
  ...
  const validLinks = getValidSocialLinks(social, siteSettings);
  ...
  return (
    <div ref={containerRef} className={`relative inline-block ${className}`}>
      {/* SOCIAL MEDIA Button Trigger */}
      <button
        id="footer-social-media-trigger"
        onClick={() => setIsOpen(!isOpen)}
        aria-haspopup="dialog"
        aria-expanded={isOpen}
        aria-label="Open social media channels popover"
        className="group min-h-[40px] inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-white/[0.05] hover:bg-[#ffbe1a] border border-white/10 hover:border-[#ffbe1a] text-slate-300 hover:text-black font-extrabold text-xs tracking-wider uppercase font-['Outfit'] transition-all duration-200 shadow-sm cursor-pointer select-none"
      >
        <Share2 className="w-3.5 h-3.5 text-[#ffbe1a] group-hover:text-black transition-colors" />
        <span>{buttonLabel}</span>
        {placement === 'top' ? (
          <ChevronUp className={`w-3.5 h-3.5 transition-transform duration-200 ${isOpen ? 'rotate-180 text-black' : 'text-slate-400 group-hover:text-black'}`} />
        ) : (
          <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-200 ${isOpen ? 'rotate-180 text-black' : 'text-slate-400 group-hover:text-black'}`} />
        )}
      </button>
      ...
      {isOpen && (
        <div
          ...
          className={`absolute left-0 z-50 w-72 sm:w-80 rounded-2xl bg-[#0e111a]/98 border border-white/15 p-3.5 shadow-[0_20px_50px_rgba(0,0,0,0.95),0_0_30px_rgba(255,190,26,0.15)] backdrop-blur-2xl animate-in fade-in zoom-in-95 duration-200 ${
            placement === 'top' 
        ...
        >
          ...
          <div className="flex items-center justify-between px-2 pb-2.5 mb-2 border-b border-white/10">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] uppercase font-bold tracking-wider text-white font-['Outfit']">
                Connect With {brandName}
              </span>
            </div>
            <span className="text-[10px] text-slate-400 font-mono">
              {validLinks.length} {validLinks.length === 1 ? 'channel' : 'channels'}
            </span>
          </div>

          {/* Social Links Grid */}
          <div className="grid grid-cols-2 gap-1.5 max-h-[300px] overflow-y-auto no-scrollbar">
            {validLinks.map((link) => {
              ...
              return (
                <a
                  ...
                  className="flex items-center gap-2.5 p-2 rounded-xl bg-white/[0.03] hover:bg-white/[0.08] border border-white/5 hover:border-[#ffbe1a]/40 text-slate-300 hover:text-white transition-all group/item"
                >
                  <div className="w-7 h-7 rounded-lg bg-white/[0.05] border border-white/10 text-slate-300 group-hover/item:text-[#ffbe1a] group-hover/item:bg-[#ffbe1a]/10 flex items-center justify-center shrink-0 transition-colors">
                    <IconComp className="w-3.5 h-3.5" />
                  </div>
                  <div className="overflow-hidden flex-1">
                    <div className="text-[11px] font-bold text-white group-hover/item:text-[#ffbe1a] transition-colors truncate">
                      {link.name}
                    </div>
                    <div className="text-[9px] text-slate-400 font-mono truncate">Official</div>
                  </div>
                  <ExternalLink className="w-3 h-3 text-slate-500 opacity-0 group-hover/item:opacity-100 group-hover/item:text-[#ffbe1a] transition-opacity shrink-0 mr-1" />
                </a>
              );
            })}
          </div>
